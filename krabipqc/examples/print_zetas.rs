...
//! `SchoolbookField::exp` + bitreversal so it can stand alone against
...
fn sb_exp(base: u32, exp: u32, q: u32) -> u32 {
...
fn main() {
    ...
    let dsa_zetas_mont: [u32; DSA_N] =
        core::array::from_fn(|i| to_mont(sb_exp(DSA_ZETA, bitrev8(i as u32), DSA_Q), DSA_Q));
    ...
    let kem_zetas_mont: [u32; 128] =
        core::array::from_fn(|i| to_mont(sb_exp(KEM_ZETA, bitrev7(i as u32), KEM_Q), KEM_Q));
    ...
    let kem_gammas_mont: [u32; 128] = core::array::from_fn(|i| {
        to_mont(sb_exp(KEM_ZETA, 2 * bitrev7(i as u32) + 1, KEM_Q), KEM_Q)
    });
